...
void xdebug_init_debugger(TSRMLS_D)
{
	...
	if (XG(remote_connect_back)) {
		...
		if (XG(remote_log_file)) fprintf(XG(remote_log_file), "I: Checking remote connect back address.\n");
	...
	if (XG(context).socket >= 0) {
		if (XG(remote_log_file)) fprintf(XG(remote_log_file), "I: Connected to client. :-)\n");
		XG(remote_enabled) = 0;
		...
		if (!XG(context).handler) {
			...
			if (XG(remote_log_file)) fprintf(XG(remote_log_file), "E: The remote debug handler '%s' is not supported. :-(\n", XG(remote_handler));
		} else if (!XG(context).handler->remote_init(&(XG(context)), XDEBUG_REQ)) {
			/* The request could not be started, ignore it then */
			if (XG(remote_log_file)) fprintf(XG(remote_log_file), "E: The debug session could not be started. :-(\n");
		} else {
			/* All is well, turn off script time outs */
			XG(remote_enabled) = 1;
		}
	} else {
		if (XG(remote_log_file)) fprintf(XG(remote_log_file), "E: Could not connect to client. :-(\n");
	}
