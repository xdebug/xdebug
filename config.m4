...
if test "$PHP_XDEBUG" != "no"; then
  ...
  if test "$PHP_XDEBUG_FOUND_VERNUM" -lt "80200"; then
    AC_MSG_ERROR([not supported. Need a PHP version >= 8.2.0 and < 8.7.0 (found $PHP_XDEBUG_FOUND_VERSION)])
  else
  ...
  XDEBUG_LIB_SOURCES="src/lib/usefulstuff.c src/lib/arg.c src/lib/cmd_parser.c src/lib/compat.c src/lib/crc32.c src/lib/file.c src/lib/hash.c src/lib/headers.c src/lib/lib.c src/lib/llist.c src/lib/log.c src/lib/normalize_path.c src/lib/set.c src/lib/str.c src/lib/stripcslashes.c src/lib/timing.c src/lib/trim.c src/lib/var.c src/lib/var_export_html.c src/lib/var_export_line.c src/lib/var_export_text.c src/lib/var_export_xml.c src/lib/xdebug_strndup.c src/lib/system_utils.c src/lib/xml.c"
