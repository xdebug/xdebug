...
require 'bug01575.inc';

DebugClient01575Manager::startListenOnBackground(ini_get('xdebug.remote_port'),sys_get_temp_dir());
sleep(1);
...
//Check (5) session with new key start twice and then stop once
...
//Check (7) session with new key start once and then stop twice
...
//Check (10) session with new key start in loop
for($i=0; $i<10; $i++) {
...
DebugClient01575Manager::stopListenOnBackground();
?>
--EXPECTF--
1: debugger inactive
2: debugger active
