...
#define SENDMSG(socket, str) {  \
	char *message_buffer;       \
                                \
	message_buffer = str;       \
	SSEND(socket, message_buffer, strlen(message_buffer)); \
...
int xdebug_php3_error(int socket, int type, char *message, char *location, uint line, xdebug_llist *stack)
{
	...
	/* start */
	SENDMSG(socket, xdebug_sprintf("%sstart: %s\n", prefix, errortype));

	/* header */
	SENDMSG(socket, xdebug_sprintf("%smessage: %s\n", prefix, message));
	SENDMSG(socket, xdebug_sprintf("%slocation: %s:%d\n", prefix, location, line));
	SENDMSG(socket, xdebug_sprintf("%sframes: %d\n", prefix, stack->size));
	...
	if (stack) {
		for (le = XDEBUG_LLIST_HEAD(stack); le != NULL; le = XDEBUG_LLIST_NEXT(le))
		{
			...
			SENDMSG(socket, xdebug_sprintf("%sfunction: %s\n", prefix, tmp_name));
			xdfree (tmp_name);
	...
	/* stop */
	SENDMSG(socket, xdebug_sprintf("%sstop: %s\n", prefix, errortype));
