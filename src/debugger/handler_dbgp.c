...
static int xdebug_fd_read_line_delim(xdebug_con *context, int block, unsigned char delim, char **cmd, int *length)
{
...
int xdebug_dbgp_cmdloop(xdebug_con *context, int block, int bail)
{
	...
	do {
		...
		ret = xdebug_fd_read_line_delim(context, block, '\0', &option, &length);
		// set it back
		if (!block) {
#ifdef WIN32
